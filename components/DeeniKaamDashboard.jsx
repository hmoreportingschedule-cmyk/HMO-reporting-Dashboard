...
export default function DeeniKaamDashboard({ onBack, onLogout, officeUser }) {
  ...
  useEffect(() => {
    ...
    try {
      // Google Drive URL se File ID nikalein. Google Sheet URL bhi supported hai.
      const fileIdMatch = sheetUrl.match(/(?:\/d\/|id=)([a-zA-Z0-9_-]+)/);
      if (!fileIdMatch) {
        throw new Error("Invalid Google Drive URL. Please paste the complete Drive file URL.");
      }
      ...
      // Approach 1: Google Sheets GViz CSV
      try {
        const gvizUrl = `https://docs.google.com/spreadsheets/d/${fileId}/gviz/tq?tqx=out:csv`;
        const res = await fetch(gvizUrl, { cache: "no-store" });
        const csvText = await res.text();

        if (res.ok && !csvText.trim().toLowerCase().startsWith("<!doctype") && !csvText.trim().toLowerCase().startsWith("<html")) {
          const parsed = Papa.parse(csvText, { header: true, skipEmptyLines: true });
          if (parsed.data && parsed.data.length > 0 && parsed.meta.fields?.length > 0) {
            data = parsed.data;
            success = true;
          }
        }
      } catch (e) { console.warn("Google Sheets GViz failed", e); }

      // Approach 2: Google Sheets CSV Export
      if (!success) {
        try {
        ...
        } catch (e) { console.warn("Google Sheets CSV export failed", e); }
      }

      // Approach 3: Google Drive file download through CORS proxies (CSV/XLS/XLSX)
      if (!success) {
        ...
        for (const proxy of proxies) {
          try {
            ...
            if (!arrayBuffer || arrayBuffer.byteLength < 20) continue;

            const firstBytes = new TextDecoder().decode(new Uint8Array(arrayBuffer).slice(0, 300)).toLowerCase();
            if (firstBytes.includes("<html") || firstBytes.includes("<!doctype") || firstBytes.includes("<form")) continue;
            ...
            console.warn("Drive proxy failed", proxy, e);
          }
        }
      }

      if (success && data.length > 0) {
        data = data.filter(r => Object.keys(r).some(k => String(r[k] ?? "").trim() !== ""));
        ...
        setFetchError("Sync fail ho gaya. Drive file ko 'Anyone with the link → Viewer' karein aur URL dobara check karein.");
      }
